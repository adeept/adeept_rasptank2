    ...
    def findLineCtrl(self, posInput):
        ...
        if posInput != None and findLineMove == 1:
            ...
            if posInput > 480: # The position of the center of the black line in the screen (value range: 0-640)
                ...
                #turnRight
                if CVRun:
                    move.video_Tracking_Move(turn_speed, 1,"right") # 'no'/'right':turn Right, turn_speed锛歭eft wheel speed, 0.2:turn_speed*0.2 = right wheel speed
                else:
                    move.motorStop() # stop
                ...
                if CVRun:
                    move.video_Tracking_Move(turn_speed, 1,"left") # 'no'/'right':turn Right, turn_speed锛歭eft wheel speed, 0.2:turn_speed*0.2 = right wheel speed
                else:
                    move.motorStop() # stop.
                ...
                if CVRun:
                    ...
                    if outv >0: 
                        ...
                        move.video_Tracking_Move(turn_speed, 1,"left",radius=(1.0-coef)) # 'no'/'right':turn Right, turn_speed锛歭eft wheel speed, 0.2:turn_speed*0.2 = right wheel speed

                    elif outv <=0: 
                        move.video_Tracking_Move(turn_speed, 1,"right",radius=(1.0-coef)) # 'no'/'right':turn Right, turn_speed锛歭eft wheel speed, 0.2:turn_speed*0.2 = right wheel speed
